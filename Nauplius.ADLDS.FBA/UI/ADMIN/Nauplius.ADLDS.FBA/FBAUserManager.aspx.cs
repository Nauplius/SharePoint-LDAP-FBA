...
namespace UI.ADMIN.Nauplius.ADLDS.FBA
{
	public partial class FBAUserManager : LayoutsPageBase
	{
		...
		private void FillTable(SearchResultCollection results)
		{
			foreach (SearchResult result in results)
			{
				var row = new TableRow {ID = "tRow" + result.Properties["objectGUID"]};
				var cell = new TableCell {Text = result.Properties["givenName"].ToString()};
				var cell2 = new TableCell {Text = result.Properties["sn"].ToString()};
				var cell3 = new TableCell {Text = result.Properties["telephoneNumber"].ToString()};
				var cell4 = new TableCell {Text = result.Properties["title"].ToString()};
				var cell5 = new TableCell {Text = result.Properties["physicalDeliveryOfficeName"].ToString()};
